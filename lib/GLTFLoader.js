	...
	class GLTFLoader extends THREE.Loader {
		...
		constructor( manager ) {
			...
			this.register( function ( parser ) {
				...
				return new GLTFSpecularGlossinessExtension( parser );
 ...
 */
	...
	/**
 * Specular-Glossiness Materials Extension
 *
 * Not an official Khronos extension — emitted by the Unreal/Maya glTF
 * exporters. Converts the legacy workflow to PBR params so the diffuse
 * (and optionally specular/gloss) maps actually load:
 *   diffuseTexture -> map (sRGB), diffuseFactor -> color
 *   specularFactor -> metalness=0 dielectric, glossiness -> roughness
 * Without this, models that only define this extension render as flat
 * untextured gray.
 */
class GLTFSpecularGlossinessExtension {
	...
	extendMaterialParams( materialIndex, materialParams ) {
		...
		if ( ext.diffuseTexture !== undefined ) {
			...
			pending.push( parser.assignTexture( materialParams, 'map', ext.diffuseTexture, THREE.sRGBEncoding ) );

		}

		if ( Array.isArray( ext.specularFactor ) ) {

			// treat as dielectric; F0 approximation via non-metalness
		...
		if ( ext.glossinessFactor !== undefined ) {
			...
			// gloss -> roughness (cheap approximation)
			materialParams.roughness = Math.min( 1.0, Math.max( 0.05, 1.0 - ext.glossinessFactor ) );

		}

		if ( ext.specularGlossinessTexture !== undefined ) {

			// .RG = specular, .B = gloss; three.js wants metallicRoughness as .B=metal,.G=rough
			// best-effort: use it as the roughness map only (specular color is rarely meaningful
			// for the low-poly characters we load)
